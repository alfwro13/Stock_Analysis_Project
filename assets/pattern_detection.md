...
**Engines:** `pattern_detection_engine.py` (registry + orchestrator), `pattern_geometry_helpers.py` (shared swing-point/pivot math + candlestick body/wick math), `head_shoulders_engine.py`, `double_top_bottom_engine.py`, `flag_engine.py`, `triangle_engine.py`, `volatility_squeeze_engine.py`, `narrow_range_engine.py`, `parabolic_stretch_engine.py`, `momentum_divergence_engine.py`, `candlestick_trigger_engine.py` (per-family detectors)
...
- **`DETECTORS`** — a `dict[str, module]` registry (`{"head_shoulders": head_shoulders_engine, "double_top_bottom": double_top_bottom_engine, "flag": flag_engine, "triangle": triangle_engine, "volatility_squeeze": volatility_squeeze_engine, "narrow_range": narrow_range_engine, "parabolic_stretch": parabolic_stretch_engine, "momentum_divergence": momentum_divergence_engine, "candlestick_trigger": candlestick_trigger_engine}`). Adding a family is adding one entry here.
- **`PatternDetectionEngine`** — the scan orchestrator. `run_scan()` builds the ticker list once (Portfolio/Watchlist per config, same scope for every family), loads each ticker's parquet once (`_LOOKBACK_BARS` = 500 trading days — widened from 180 once Parabolic Stretch's 200-day SMA + 252-day Z-score window needed most of the 2-year parquet history; every other family only ever searches the most recent bars regardless of how much earlier history is available, so the wider window is a no-op for them), computes RSI/volume-SMA once, then calls every enabled registered detector's `detect(...)` against that same data and collects results. `_analyse_ticker()` also tracks, per ticker, every family whose `detect()` ran cleanly but returned `None` (as opposed to raising) — `_save_results()` upserts each positive result into `pattern_detection_results` (keyed on `ticker, pattern_family`, so families never clobber each other) **and deletes any existing row for a `(ticker, family)` pair that came back empty this scan**, so a pattern that's no longer detected doesn't linger as "active" indefinitely (a family that raised is left untouched — a transient failure isn't evidence the pattern ended). It also logs to `pattern_detection_history`, skipping a duplicate log row when the geometry and phase are unchanged from the previous scan. This stale-row cleanup is what makes a single-bar family like Candlestick Trigger (§7d) actually stop showing as active the day after it fires — without it, `detect()` returning `None` would just mean no new row is written, leaving the previous day's row untouched forever.
...
- `candle_body_wick_metrics(open, high, low, close)` / `is_bullish_engulfing` / `is_bearish_engulfing` / `is_hammer` / `is_shooting_star` — single-bar body/wick decomposition and the four Candlestick Trigger shape tests (§7d). Shared with `quant_signals.py`'s older Quantamental scoring feature, which has its own independent implementation of the same shapes for a different purpose (a per-scan composite score, not a persisted Pattern Detection result) — both call into these functions rather than maintaining two copies of the same body/wick math.
...
`points` and `lines` are what make the schema and chart renderer geometry-agnostic: Head & Shoulders returns 5 points and 1 (neckline) line; Double Top/Bottom returns 3 points and 1 (support/resistance) line; Flag returns 2 points (pole start/end) and 2 (upper/lower channel) lines; Triangle returns its swing-high/swing-low touch points (count varies — at least 2 per side) and 1 (flat resistance or support) line; NR4/NR7 Narrow Range returns 2 points (the narrow bar's high/low) and 2 (breakout trigger) lines; Parabolic Stretch returns 2 points (the stretch bar and today) and 1 `path`-based line (the SMA curve between them, §4b); Bullish/Bearish Divergence returns 3 points (two extremes + the middle pivot) and 1 (middle-pivot level) line, the same shape as Double Top/Bottom; Candlestick Trigger returns 4 points (Engulfing's two candle bodies) or 2 points (a Pin Bar's body edge and wick tip) and 1 (support/resistance/rejection level) line, all confined to the trigger's own 1-2 bars. `pattern_detection_detail.js`'s chart renderer (`_pdBuildPatternTraces`) iterates whichever `points`/`lines` a result carries and draws a filled shape through `points` (closed back to the first point) plus the `lines` as key-level segments — it needs no changes to support a new family's shape, as long as that shape is expressible as a handful of discrete points plus straight line segments.
...
**Symmetrical Triangle (`triangle_engine.py`, added 2026-07-22):** the third variant in the same family, sharing the module's `_ORDER`/`WINDOW_DAYS`/`find_pivots`/`linreg` machinery but combining Ascending/Descending's regression-based validation with §7b's phase model rather than either alone — neither side is flat here, so unlike Ascending/Descending (whose `pattern_type` implies a direction from the moment it's detected), the breakout direction is genuinely unknown until price actually breaks one of the two lines.
- *Both sides sloped, converging:* the swing-high line must be falling (`<= -MIN_SLOPE_PCT`, the same threshold Ascending/Descending's sloped side already uses) **and** the swing-low line must be rising (`>= MIN_SLOPE_PCT`) over the same window — this combination is what rules out a wedge (both lines sloping the same direction) or a widening megaphone (the lines diverging instead of converging). An additional check that the resistance line sits above the support line both at the back of the window and as of today rejects a candidate whose lines have already crossed (the apex has passed) or never actually bracketed a range.
- *FORMING:* today's close sits between the two lines' current (regression-extrapolated) values — `pattern_type = "symmetrical_triangle"`, no direction, mirroring Volatility Squeeze's neutral state.
- *CONFIRMED:* today's close is decisively above the resistance line's current value (`symmetrical_triangle_bullish`) or below the support line's current value (`symmetrical_triangle_bearish`) — no separate lookahead window is needed the way Volatility Squeeze/Narrow Range require one, since the two lines are evaluated fresh on every scan rather than a state that can expire.
- *Volume/RSI:* the same declining-volume-over-the-window check as Ascending/Descending (a soft supporting signal, not a hard gate); RSI compared from the back of the window to the breakout bar (or today, while still forming), in the resolved direction once one exists.
- *Target:* projects the triangle's height — the gap between the two regression lines at the back of the window, its widest point — from the breakout point, in the resolved direction.
- *Toggles:* `BULLISH_ENABLED`/`BEARISH_ENABLED` (reusing the same generic field names Volatility Squeeze/Narrow Range/Momentum Divergence already use in `PatternFamilyScheduleConfig`) gate which confirmed breakout direction gets reported — the FORMING state is always searched, same convention as §7b.
...
## 7d. Candlestick Trigger — 1-2 Bar Execution Triggers With No FORMING Phase

Unlike every other family, Candlestick Trigger (`candlestick_trigger_engine.py`) doesn't describe a multi-day shape at all — it's a set of 1-2 bar micro-structure triggers meant to *confirm* a reversal a larger setup (a Double Bottom, a support retest, etc.) has already flagged, not to stand alone as structural analysis. `detect()` only ever looks at `df`'s last bar (plus the one before it for Engulfing) and only ever returns `phase = "CONFIRMED"` — there is no FORMING state, since the pattern is either present as of today's bar or it isn't. This makes the family naturally single-bar and self-expiring: the day after a trigger fires, `today_idx` has moved on and the same candle no longer qualifies as "today's" bar, so `detect()` correctly returns `None` again — relying on `_save_results()`'s stale-row cleanup (§2) to actually clear the previous day's row rather than leaving it shown as active.

Each of the four `pattern_type`s only qualifies when price is already near an extreme — this is the "trigger, not standalone signal" design: the same candle shape occurring mid-range, with nothing to confirm, is treated as noise.

- **Bullish/Bearish Engulfing:** the prior candle must be the opposite color of the current one, and the current candle's body must fully contain the prior candle's (`curr_open <= prev_close` and `curr_close >= prev_open` for bullish, mirrored for bearish) — see `pattern_geometry_helpers.is_bullish_engulfing`/`is_bearish_engulfing`. The direction check is required alongside the containment inequalities; containment alone also holds for candle pairs that aren't a genuine reversal engulfing.
- **Hammer/Shooting Star (Pin Bars):** a rejection wick at least `WICK_MULTIPLIER` (default 2.0) times the real body, with the opposite wick no larger than `OPPOSITE_WICK_MAX_PCT` (default 20%) of the bar's total range — see `pattern_geometry_helpers.is_hammer`/`is_shooting_star`. Hammer requires the long wick on the lower side (rejecting an intraday low); Shooting Star requires it on the upper side (rejecting an intraday high).
- **Context gate (all four types):** a bullish trigger only qualifies when price closes below the lower Bollinger Band (`indicators.compute_bollinger_bands`, `BB_WINDOW_DAYS`/`BB_NUM_STD`, default 20/2.0) **or** RSI is at or below `RSI_OVERSOLD` (default 30) — mirrored above the upper band / at or above `RSI_OVERBOUGHT` (default 70) for a bearish trigger. The result's `rsi_divergence` field records whether the RSI half specifically held (not just the gate overall), so a trigger that qualified only via the Bollinger Band still surfaces an honest signal rather than always reporting `True`.
- **Priority when multiple shapes fire on the same bar:** checked in a fixed order — bullish engulfing, bearish engulfing, hammer, shooting star — and the first passing candidate is returned, since only one result can be stored per `(ticker, pattern_family)`. Bullish and bearish variants can't both qualify simultaneously (they require opposite RSI/Bollinger extremes and opposite candle directions), so in practice this only matters for a same-direction Engulfing/Pin-Bar overlap.
- **Target/volume:** `measured_target` projects the engulfing candle's own body height (Engulfing) or the rejection wick's own length (Pin Bar) from the close; `volume_confirms` requires the trigger bar's volume to exceed its 20-day average by `VOLUME_CONFIRM_MULTIPLIER` (default 1.5).
- **Toggles:** `ENGULFING_ENABLED`/`PIN_BAR_ENABLED` gate which shape family is searched at all; `BULLISH_ENABLED`/`BEARISH_ENABLED` gate which direction gets reported, same convention as every other family.

`quant_signals.py`'s older Quantamental scoring feature (the composite per-ticker score, unrelated to Pattern Detection) already had its own independent implementation of these same four shapes for its own per-scan candlestick scoring — both now call into `pattern_geometry_helpers`'s shared functions rather than maintaining two copies of the same body/wick/containment math (AGENTS.md rule 17).

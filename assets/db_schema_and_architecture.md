...
* **Purpose:** Per-day snapshot of each built-in account's value, used to draw the account-value-over-time chart on the account detail page, and as the baseline history for `accounts_engine.period_returns()`'s 1D/1W/1M/3M/6M/1Y figures. Written nightly by the scheduled job, but also recomputed immediately (via `accounts_engine.resnapshot_account()`) after every transaction add/edit/delete, transfer, or Ghostfolio import, so the chart never waits a full day to reflect a change. `snapshot_all_accounts()` isolates each account in its own try/except so one account's pricing failure (e.g. a delisted ticker) can't abort the snapshot for every account processed after it that night — a prior all-or-nothing loop left later accounts with a stale baseline (silently wrong period returns) until a manual "Account Value Snapshot" re-run.

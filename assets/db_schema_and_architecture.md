...
* **Key Columns:** `id` (PK autoincrement), `account_id` (FK to accounts), `buy_txn_id` (FK to the `account_transactions` Buy row), `ticker` (`TBILL-{buy_txn_id}`, `UNIQUE`), `face_value` (par/maturity value), `purchase_price` (discount price actually paid — always `< face_value`), `purchase_date`, `maturity_date`, `auto_reinvest` (bool — a reminder-only flag, see below), `status` (`Open` | `Matured`), `maturity_txn_id` (FK to the `account_transactions` Sell row posted at maturity, `NULL` until then), `notes`, `created_at`.

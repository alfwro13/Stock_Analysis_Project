...
* **Key Columns:** `ticker` (PK), `name`, `price`, `change_pts`, `change_pct`, `is_positive`, `last_updated`, `market_state`. `market_state` is populated for every exchange-status proxy ticker in `market_pulse._MARKET_STATUS_PROXY` (`^GSPC` NYSE, `^FTSE` LSE, `^GDAXI` XETRA, `^N225` TSE, `^HSI` HKEX, `000001.SS` SSE, `^AXJO` ASX, `^FCHI` Euronext — extended 2026-07-08 alongside the Markets page) with the live Yahoo `marketState` value (`"REGULAR"`, `"CLOSED"`, `"PRE"`, `"POST"`, ...); `market_pulse.is_exchange_open(exchange)` reads it to give `GET /api/system/market-status`/`GET /api/system/market-status/all` and `accounts_engine.tickers_needing_refresh()` an exchange-holiday-aware open/closed check, falling back to `time_engine.is_trading_session()`'s weekday/hours heuristic for any exchange without a proxy, or when no live state is cached yet. `GET /api/system/market-status`/`market-status/all` self-trigger a `fetch_and_save_pulse()` background task for any proxy ticker whose `market_state` is missing or older than 5 minutes (`market_pulse.proxy_tickers_needing_refresh()`), so a caller that only ever polls those endpoints (e.g. Home Assistant, never opening a browser dashboard) still keeps `market_state` warm instead of it going stale forever.
...
* **Purpose:** One row per `Trading` account holding its last computed live-performance figures — total value, equity value, cash balance, unrealized P&amp;L, realized P&amp;L, dividend/interest income, 1D/1W/1M/3M/6M/1Y period gain/loss in `BASE_CURRENCY`, and since-inception Money-Weighted Rate of Return (Modified Dietz method, %) — powering the live tile rows on the Trading account detail page and the Home Assistant `account_metrics_list()` per-account sensor set. Written by `accounts_engine.refresh_performance_cache()`, called from two places sharing the one canonical loop `accounts_engine.refresh_all_trading_performance_caches()`: as a side effect of the Crash/Moonshot scan (`intraday_orchestrator_job`, every `CRASH_ALERTS.INTERVAL_MINUTES`, which already fetches live prices for every held ticker into `market_pulse_cache`), and — added 2026-07-07 to keep these figures closer to live — the dedicated, always-on `account_performance_refresh_job` (mon-fri 07:00–21:59 UTC, every 1 minute; DB-only re-derivation from the already-cached prices, no Yahoo Finance calls, so it's safe at that cadence independent of the heavier scan's own interval). So the figures are computed server-side and shared by every browser/tab that polls `GET /api/accounts/{id}/live-performance`, rather than each poll re-deriving MWRR/period-returns from the full transaction history. Falls back to computing on the fly (still writing the row) if a request arrives before the next scan has populated it — e.g. a brand-new account. `realized_pnl`/`dividend_income`/`interest_income` are derived from the same `account_summary()` call as the other fields in that refresh, so `account_metrics_list()` can read every field from this one cache row instead of mixing it with a separately-timed live `account_summary()` call.
...
* **Purpose:** Stores an optional low/high price alert limit per (account, ticker) holding — a.k.a. "Set Targets" on the Stock Detail page's Your Position box, and the Home Assistant integration's Phase 3 Low Limit / High Limit number entities. Both surfaces read/write the same rows, so a target set in one place shows up in the other. Powers the `low_limit_set`/`low_limit_reached`/`high_limit_set`/`high_limit_reached` fields on `GET /api/accounts/holdings-list`, and (added web UI + notifications) is polled every 5 minutes during market hours by `intraday_orchestrator.py`, which fires a `holding_limit_alert` notification the first time a held ticker's live price crosses a stored limit. Scoped to built-in accounts only (`account_id` FK) — a Ghostfolio-only holding has no row to attach a target to.
* **Key Columns:** `account_id` (FK to accounts), `ticker`, `low_limit`, `high_limit` (both nullable — either can be set independently of the other; a client sets a column to `NULL` to clear/unset that target, e.g. the Stock Detail page's Set Targets panel treats an empty or `0` input as "clear"), `updated_at`.
...
* **Notification dedup:** `intraday_orchestrator._evaluate_alert_gate()`/`record_alert_fired()` key on a composite `alert_state.ticker` value `{account_id}:{ticker}:low` / `{account_id}:{ticker}:high` (not a bare ticker symbol), since the same real ticker can independently breach a low and a high target, each needing its own cooldown/rearm state. Config knobs live in `NOTIFICATIONS.HOLDING_LIMIT_ALERTS` (`config.py`).

...
* **`face_value` starts as an estimate, confirmable once known:** Freetrade never states a bill's face value directly — only the amount debited and an indicative, pre-tender yield (the real yield for a specific bill isn't fixed until the Friday DMO tender closes, which can be after the bill is already logged in this app). The Buy T-Bill modal computes `face_value = purchase_price + purchase_price × indicative_ytm × days/365` (`treasury_bill_engine.estimate_face_value()`) and pre-fills it as an editable field. Once `purchase_date` (the settlement date, always after that Friday's tender) has arrived, an unconfirmed bill surfaces a "Confirm the final YTM" banner on the account page (`treasury_bill_engine.bills_pending_ytm_confirmation()`, mirroring the Auto Top-up confirm/dismiss pattern) — entering the real rate recomputes `face_value` with the same formula and stores both (`treasury_bill_engine.confirm_ytm()`); "Keep Estimate" just sets `ytm_confirmed=1` unchanged. Either way this is a one-time confirmation, not re-editable afterward through the UI (delete and re-enter if it turns out wrong even after confirming).

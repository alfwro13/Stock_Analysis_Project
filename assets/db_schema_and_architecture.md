...
* **`face_value` is an operator-confirmed estimate, not a fetched fact:** Freetrade never states a bill's face value directly — only the amount debited and an indicative, pre-tender yield (the real yield for a specific bill isn't fixed until the Friday DMO tender closes). The Buy T-Bill modal computes `face_value = purchase_price + purchase_price × indicative_ytm × days/365` (`treasury_bill_engine.estimate_face_value()`) and pre-fills it as an editable field, so the operator can hand-correct it before saving if they later know the exact redemption amount. Whatever value is saved is treated as exact from that point on — it is never re-derived from `indicative_ytm` at maturity time, since the app has no way to learn the real confirmed yield after the fact.

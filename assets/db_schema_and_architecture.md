...
* **Key Columns:** `id` (PK autoincrement), `account_id` (FK to accounts), `price_date` (YYYY-MM-DD), `price`, `source` (`scrape` | `csv_import` — distinguishes a scheduled/manual scrape from a pasted historical-CSV backfill), `created_at`.

...
```json
{
  ...
  "views": [
    ...
    {"name": "Oversold", "columns": ["ticker", "price", "rsi"], "filter": [{"key": "rsi", "operator": "lt", "value": "30"}]}
  ]
}
```
...
At-a-glance market/system status for the Home Assistant integration's sensors. `us_market_open`/`uk_market_open` use `market_pulse.is_exchange_open("NYSE"/"LSE")`, which first vetoes via `time_engine.is_exchange_holiday()` (backed by `exchange_calendars`, added 2026-07-13 — the one canonical holiday source for every exchange, not just these two), then prefers the live Yahoo `marketState` cached from that exchange's proxy index ticker (`^GSPC` for NYSE, `^FTSE` for LSE, refreshed into `market_pulse_cache.market_state` by `fetch_and_save_pulse()`) — `market_state == "REGULAR"` means open — falling back to `time_engine.is_trading_session(exchange)` (itself holiday-vetoed, and — as of 2026-07-13 — using the actual daily open/close `exchange_calendars` reports for NYSE/LSE, including early closes like the day after Thanksgiving, rather than a fixed static hour) only when no live state has been cached yet (e.g. immediately after a fresh install, before the first market-pulse fetch). This endpoint self-triggers a background `fetch_and_save_pulse()` for whichever of the two proxy tickers has a missing/stale (>5 min) `market_state` (`market_pulse.proxy_tickers_needing_refresh()`) — the same self-refresh pattern `GET /api/market-pulse` and the accounts endpoints already use — so a caller that only ever polls this endpoint (e.g. Home Assistant, with no browser dashboard open to drive `/api/market-pulse`'s own JS polling) still keeps the cache warm rather than depending on something else fetching those two tickers. It also warms the full Markets registry (`market_pulse.registry_tickers_needing_refresh(markets_engine.registry_lookup_tickers())`) — unlike the proxy self-heal above, an already-cached registry ticker is additionally gated on `market_pulse.is_ticker_quote_settled()` for its own exchange (added 2026-07-12), not refreshed on age alone, so this poll can't itself pull a not-yet-representative delayed quote the instant a tile's exchange opens; see `assets/markets_page.md` §10. `yahoo_ok` reflects real Yahoo Finance call history from `database.get_yahoo_api_stats()` (at least one non-error call recorded on the most recent tracked day), not the in-process cache hit-rate, which reads 0% right after every server restart regardless of Yahoo's actual health. `system_ok` is `len(issues) == 0` from the same System Check Engine used by `GET /api/system/checks`.
...
Aggregates live figures across every non-deleted Trading account — the Home Assistant integration's portfolio-summary sensor data source. Thin wrapper around `portfolio_metrics_engine.portfolio_totals()`. With zero Trading accounts, returns the same shape with all monetary fields at `0.0` and all percentage/return fields `null` rather than erroring.

Before reading, this endpoint also checks whether any held ticker's `market_pulse_cache` price is older than `UI_PREFERENCES.REFRESH_RATE` (while a relevant market is open) and, if so, kicks off a background `market_pulse.fetch_and_save_pulse()` for whichever tickers are due — the same `needs_refresh` pattern `GET /api/market-pulse` already uses for the live-ticking widget, extended to cover every held ticker rather than only ones rendered on screen. This means polling this endpoint (e.g. from Home Assistant) at a given interval makes that interval the real data-refresh cadence, not just a read cadence — see `accounts_engine.tickers_needing_refresh()`. A ticker with no cached row at all is refreshed even while both markets are closed (a fresh install, a newly-bought ticker, or a maintenance-prune gap must be able to self-heal without waiting for a market to reopen); only a ticker that already has *some* cached data skips the refresh while markets are shut, since its price can't have moved either way. An already-cached ticker is also gated on `market_pulse.is_quote_settled(exchange)` (added 2026-07-10) — Yahoo's free LSE feed runs ~15-20 minutes behind the real market, so a ticker whose exchange has *just* opened is deliberately left on its last cached price rather than refreshed immediately, avoiding a not-yet-representative delayed quote from producing a visibly wrong, self-correcting blip in the account gain figures HA reads right at market open.

The Home Assistant integration itself polls this endpoint (and `list-with-metrics`/`holdings-list` below) on every cycle regardless of market hours by default (`CONF_SKIP_REFRESH_WHEN_MARKETS_CLOSED`, off by default in the HA integration's own config) — the market-open/quote-settled gating above already makes it safe and cheap to do so, so the integration no longer needs to protect against it on its own side.
...
**Response:** `{ "status": "success", "message": "Refresh complete." }` once the refresh has finished, or a `500` with `{ "status": "error", "message": ... }` on failure; check the Notifications panel either way for a persistent record.
...
Per-holding metrics across every non-deleted Trading account, for the Home Assistant integration's Phase 3 per-holding sensor (one HA device per (account, ticker) pair — the same ticker held in two accounts produces two separate rows, never merged). Thin wrapper around `portfolio_metrics_engine.holdings_with_metrics_all_accounts()`, which reuses `holdings_with_market_value()` per account and enriches each row with native price (`accounts_engine.current_price_map()`), technicals from `stock_signals` (RSI, 50d/200d trend, next earnings date), 24h change from `market_pulse_cache`, accumulated dividends (summed from `Dividend`-type transactions for that ticker in that account), and any stored price limit from `holding_price_limits`. With zero Trading accounts, returns `"holdings": []`.

`TBILL-%` rows (synthetic UK Treasury Bill tickers, see the `treasury_bills` table) are the one exception to the `stock_signals`/`market_pulse_cache` sourcing above — neither table ever has a row for a synthetic ticker, so `asset_class`/`market_change_24h`/`market_change_pct_24h` would otherwise always be `null`. Instead `asset_class` is fixed to `"Fixed Income"` and `market_change_24h`/`market_change_pct_24h` are computed analytically via `treasury_bill_engine.daily_accretion_change()` (today's `accreted_price()` minus yesterday's) — never `null` for a TBILL row.

Like `portfolio-totals` above, this endpoint also self-triggers a background live-price refresh for any held ticker whose `market_pulse_cache` row is due (see that endpoint's note). `market_price`'s own freshness comes from `current_price_map()` preferring `market_pulse_cache` unless it's missing or has fallen more than 6h behind `stock_signals.last_updated` — not gated by an absolute-age cutoff on the cache side, since the background jobs that keep `market_pulse_cache` warm run far coarser than any UI poll rate, and a price a few minutes old is still far better than the once-nightly `stock_signals` fallback it would otherwise revert to. See `assets/db_schema_and_architecture.md`'s `market_pulse_cache` entry for the 2026-07-13 fix that replaced a bare "whichever timestamp is newer" comparison with this minimum-gap requirement.
...
Current value + basic performance for every non-deleted Pension/House account, for the Home Assistant integration's Phase 4 "Other Accounts" sensors (one sensor per account). Thin wrapper around `portfolio_metrics_engine.other_accounts_list()`. With zero Pension/House accounts, returns `"accounts": []`.
...
Sets one holding's low and/or high price alert limit ("Position Targets" — a standalone box on the Stock Detail page, independent of the Your Position box — and the Home Assistant integration's per-holding Low Limit / High Limit number entities — both read/write the same `holding_price_limits` row, so a target set on either surface is reflected on the other). Thin wrapper around `portfolio_metrics_engine.set_holding_price_limit()`. `account_id` may be any built-in account, including the singleton Watchlist account (`db_accounts.get_watchlist_account()`) — a watchlisted-but-not-held ticker can carry its own target. Every 5 minutes during market hours, `intraday_orchestrator.py`'s scan compares each targeted ticker's live price against any stored limit (low fires on `current_price <= low_limit`, high on `current_price >= high_limit`) and fires a `holding_limit_alert` notification (source label "Position Target Reached") the first time it's crossed each UTC calendar day, using a dedicated once-per-day `alert_state` gate (composite key `{account_id}:{ticker}:low`/`:high` since low and high targets on the same ticker are independent conditions) — deliberately simpler than Crash/Moonshot's worsened/recovered/cooldown model, since a static price threshold can legitimately be crossed back and forth several times in one session and only the first crossing per day should notify — plus its own independent notification-channel routing. A ticker with a target but no actual holding is scanned only for this check (`IntradayOrchestrator._compute_target_only_tickers()`), never entering Crash/Moonshot/Anomaly evaluation. `low_limit`/`high_limit` are in the ticker's **native Yahoo units** — raw pence for a `GBp` (LSE) ticker, matching `current_price` — never GBP pounds; the Stock Detail page's Position Targets input box converts pounds↔pence at the UI boundary before/after calling this endpoint (see `holding_price_limits` in `assets/db_schema_and_architecture.md`), and any other caller (including the Home Assistant integration) must do the same conversion itself if it wants to accept/display pounds.
...
`regions` values are `open` (every constituent exchange open), `partial` ("Some Open"), `pre`, or `closed` — see `markets_engine.get_region_state()`. Every exchange is holiday-aware via `time_engine.is_exchange_holiday()` (backed by `exchange_calendars`, added 2026-07-13), consulted by `market_pulse.is_exchange_open()` as a veto ahead of its Yahoo `marketState` proxy lookup for NYSE/LSE/XETRA/TSE/HKEX/SSE/ASX/Euronext, and inside `time_engine`'s own weekday+hours heuristic for every other exchange — which itself now sources the day's actual open/close from `exchange_calendars` (`time_engine.market_window_utc()`), correctly shortening the window on a real early-close/half-day rather than assuming the static full-day hours in `exchange_hours.json`.

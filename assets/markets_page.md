...
**Navigation (added 2026-07-09):** the Market Screener and Market Reports links were moved off the top navbar and onto the Markets page's own header row (`templates/markets.html`), immediately after the Live Markets Data indicator — Markets is now the entry point for both, rather than three separate top-level nav items. `/market-screener` gained a `← Back` button (`javascript:history.back()`, matching `stock_detail.html`'s pattern) since it's no longer always one navbar click away. The top navbar itself was also reordered: Accounts now sits directly after Portfolio (`Markets, Portfolio, Accounts, Watchlist, News, Earnings Volatility, Market Sentiment, Tools, Settings, Glossary, Notifications`).

**Market Reports split (2026-07-17):** the Markets page's "Market Reports" header link now points to the `/reports` hub instead of a single `/market-reports` page — `market_reports.html`/`market_reports.js` were retired in favour of 7 standalone report pages (one guide-card each on `/reports`), see `assets/reports_page.md`.
...
`markets_engine.get_exchange_state(exchange) -> "open"|"pre"|"closed"` is built on `market_pulse.is_exchange_open()`, which is holiday-aware for every registry exchange via `time_engine.is_exchange_holiday()` (backed by the `exchange_calendars` library — added 2026-07-13, see §11), consulted as a veto ahead of everything else. Beyond that veto, 8 exchanges — NYSE, LSE, XETRA, TSE, HKEX, SSE, ASX, Euronext — additionally get live intraday session granularity (PRE/POST/REGULAR/CLOSED) from a Yahoo `marketState` proxy ticker; anything else falls back to `time_engine`'s weekday+hours heuristic for the open/closed distinction (now itself holiday-vetoed too).
...
- **Fixed 2026-07-13 — all 29 registry exchanges are now genuinely holiday-aware, not just NYSE/LSE et al. via the Yahoo proxy.** `time_engine.is_exchange_holiday(exchange)` (backed by the `exchange_calendars` library) is consulted as an explicit veto by `market_pulse.is_exchange_open()`/`get_exchange_session_state()` ahead of the Yahoo-`marketState`-proxy lookup, and by `time_engine.is_market_open()` for every exchange without a proxy. 26 of the 29 exchanges in `exchange_hours.json` map to a dedicated `exchange_calendars` calendar; NSE and SZSE deliberately share their sibling exchange's calendar (BSE/SSE respectively — same national holiday calendar), and the `Euronext` bucket (Paris/Amsterdam/Brussels/Lisbon) uses Paris's calendar (Euronext has run one harmonised holiday calendar across venues since 2002). The Yahoo-marketState proxy is retained for its original purpose only — live intraday PRE/POST/REGULAR/CLOSED granularity for the 8 proxied exchanges — not as a holiday source.
- **Fixed 2026-07-13 — `time_engine.market_window_utc()` now also sources the *actual* daily open/close from `exchange_calendars` for every mapped exchange, not just whether today is a holiday.** This catches early-close/half-day sessions the static `exchange_hours.json` file can't represent (e.g. NYSE's post-Thanksgiving and Christmas Eve 13:00 ET closes) — `time_engine._session_window_override()` supplies the day's real `(open, close)` when the exchange has a calendar mapping and today is a valid session, falling back to the static hours otherwise (unmapped exchange, or any lookup error). The `premarket_open` key (NYSE only) is unaffected — `exchange_calendars` has no concept of an extended/premarket session, so that branch stays static. Building this surfaced 4 small pre-existing inaccuracies in `exchange_hours.json` (BOVESPA, OMXH, OSE, TASE — off by 5–60 minutes), corrected in the same change; a permanent regression test (`tests/test_time_engine.py::TestSessionWindowOverride`) now checks all 29 exchanges' static hours against the calendar on an ordinary day so the two can't silently drift apart again. `db_schema.py:_DEFAULT_EXCHANGE_HOURS` (the fresh-install seed) was corrected identically, since its backfill logic only fills in missing keys and never overwrites an existing value on an already-seeded install.

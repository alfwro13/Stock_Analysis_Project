...
def test_fill_smgb_actual_computes_error_metrics():
    ...
    _db.fill_smgb_actual("2099-02-01", actual_open=5.1)
...
def test_fill_smgb_actual_wrong_direction():
    ...
    _db.fill_smgb_actual("2099-02-02", actual_open=5.2)       # actual UP
...
def test_fill_smgb_actual_missing_row_does_not_raise():
    ...
    _db.fill_smgb_actual("2099-99-99", actual_open=1.0)  # must not raise
...
def test_get_smgb_accuracy_empty_returns_structure():
    """get_smgb_accuracy must always return a dict with 'rows' and 'summary' keys."""
...
def test_get_smgb_accuracy_summary_fields_present():
    """summary dict must have all expected keys regardless of data."""
    summary = _db.get_smgb_accuracy()["summary"]
...
def test_get_smgb_accuracy_counts_resolved_rows():
    """resolved_count must equal the number of rows with actual_open populated."""
    conn = _conn()
    try:
        conn.execute(
            "INSERT OR IGNORE INTO smgb_predictions "
            "(prediction_date, target_date, predicted_price, actual_open) "
            "VALUES ('2099-03-01', '2099-03-02', 5.0, 5.1)",
        )
        conn.commit()
        result = _db.get_smgb_accuracy()

...
@pytest.mark.api
def test_earnings_volatility_accuracy_returns_200(client):
    """GET /api/earnings-volatility/accuracy must return 200 with by_ticker/overall shape."""
    resp = client.get("/api/earnings-volatility/accuracy")
    assert resp.status_code == 200, f"Expected 200, got {resp.status_code}"
    data = _json(resp)
    assert data.get("status") == "success", f"Expected success status: {data}"
    assert "by_ticker" in data

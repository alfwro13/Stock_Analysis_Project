...
* **Head & Shoulders Pattern Detector:** Scans Portfolio and Watchlist tickers once daily (`/head-shoulders`) for Head & Shoulders and Inverse Head & Shoulders chart patterns using rolling-window swing-point detection. A candidate is only accepted if the head genuinely exceeds both shoulders, the shoulders are roughly balanced in height and timing, and — critically — a genuine prior trend preceded the left shoulder (an uptrend for the regular topping pattern, a downtrend for the inverse bottoming pattern), to reduce false positives on sideways price action. Each candidate is checked against three supporting signals: volume should decline through the shoulders and head then spike on the neckline breakout, RSI divergence at the head confirms weakening momentum, and a pattern fit-quality score (R²) ranks how cleanly price tracked the idealised shape. A pattern shows as **Forming** while price sits on the safe side of the neckline and **Confirmed** once price breaks through it, at which point a measured-move price target is calculated. A one-time "Backfill Historical Data" action walks two years of existing price history to populate a prediction-accuracy panel before relying on live alerts. Scope (Portfolio default-on, Watchlist optional), detector toggles, and alert thresholds are configurable in Settings → Tools → Head & Shoulders Pattern Detector.

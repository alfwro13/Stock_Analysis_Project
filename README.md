...
* **Change Period (1D/5D/1M/6M/YTD/1Y):** The Portfolio and Watchlist tables' Change column can be toggled between six lookback windows matching Yahoo Finance's own ranges — 1D stays live off the intraday price feed as before, while 5D/1M/6M/YTD/1Y compare the live price against the closest available close for that calendar cutoff. On the Portfolio page the heatmap updates alongside the table. Switching periods remembers your last-selected period on your next visit (tracked separately per page).
* **Pre-Market / After-Hours Prices:** Market Price, P&L, and portfolio totals always reflect the last completed regular-session close — never a pre-market or after-hours tick — determined directly from Yahoo Finance's own market-session flag rather than guessed from the time of day. A **Show After Market Data** switch next to the Portfolio page's Change Period buttons optionally reveals the latest pre-market/after-hours price in brackets under each holding's Market Price, purely for information; the Stock Detail page always shows a Pre-Market/After Hours line when one is currently active for that ticker, with no toggle needed. When the switch is on, the Portfolio Heatmap also shows a compact pre/post-market change line on tiles large enough to fit it, with the full detail available as a tooltip on smaller tiles.
...
* **Alert Confidence Referee (pilot):** A meta-labeling classifier, piloted on the Market Trap & Recovery Monitor, that learns from an alert engine's own historical hit/miss record and can veto a new alert that resembles past false positives — distinct from the existing alert cooldown/dedup gate, which only stops the *same* condition re-firing. Configurable in Settings (Alerts & Reports column): enable/schedule weekly training, choose Shadow (log-only) or Active (enforcing) mode, and set the veto threshold and minimum training-sample count. Active mode only takes effect once enough resolved Trap Monitor history has accumulated; below that the referee stays in Shadow mode regardless of the configured setting, logging what it would have done. A readiness panel shows current vs. required sample count and an estimated readiness date, with a "View Shadow Log" link opening the most recent evaluations (ticker, phase, fire probability, would-veto flag) for inspection.
...
* **Portfolio Heat Index (Risk Orchestrator):** A single 0-100 traffic-light score (`/portfolio-heat-index`, linked from the Reports menu) synthesizing portfolio VaR, max pairwise correlation, and max drawdown into one reading per account scope — computed daily and shown as a compact widget on the Portfolio page (all accounts combined) and each Account Detail page (that account alone). A separate per-ticker **Risk Contribution** tier (marginal VaR share, correlation exposure, distance from its ATR stop) shows as a new "Heat Index" column on the Portfolio page and a row on each held ticker's Stock Detail page. When the combined-portfolio reading turns Red, BUY-flavored signals get a "Risk Paused" warning badge across the Portfolio, Watchlist, and Stock Detail pages — a visual warning only, since this app never executes trades automatically. Weights and thresholds fully configurable in Settings. A **Pre-Trade Check** in the Stock Detail page's Position Sizing panel runs the same scoring on demand: as you size a position, it simulates what adding it would do to portfolio VaR and correlation and shows an Approve/Warn/Reject verdict (with a suggested smaller size on Warn/Reject) — advisory only, since there's no order execution to actually block. An optional **Daily Rollup** notification summarizes the current Heat Index per scope, VaR, and any ATR stops that moved up since the prior trading day, on its own configurable schedule. Optional **Critical Escalations** fire an instant alert when a scope's PHI or max correlation reaches Red, or a held position's live price breaches its ATR stop — configurable enable/cooldown in the same Settings card.
...
* **Pattern Detection:** Scans Portfolio and Watchlist tickers once daily (`/pattern-detection`) for chart patterns using rolling-window swing-point detection, dispatched through a per-pattern-family detector registry so new pattern types can be added without touching the database schema, scheduler job, or chart renderer. Covers reversal patterns — **Head & Shoulders** / **Inverse Head & Shoulders** (a candidate is only accepted if the head genuinely exceeds both shoulders, the shoulders are roughly balanced in height and timing, and a genuine prior trend preceded the left shoulder) and **Double Top** / **Double Bottom** (two peaks or troughs of near-equal extreme separated by one qualifying opposite pivot) — continuation patterns — **Bull Flag** / **Bear Flag** (a sharp flagpole move exceeding a volatility-scaled threshold, followed by a brief consolidation whose swing highs and lows form two roughly parallel, mildly counter-sloped channel lines), **Bull Pennant** / **Bear Pennant** (the same flagpole prerequisite as a Flag, but the consolidation is a short converging micro-triangle rather than a parallel channel), **Ascending Triangle** / **Descending Triangle** (a flat resistance or support line against a rising or falling opposite side), and **Symmetrical Triangle** (a falling resistance line and a rising support line converging together, directionally neutral until price breaks decisively past either one) — exhaustion/convergence patterns — **Rising Wedge** / **Falling Wedge** (both trendlines slope the same direction, but converge because one line moves faster than the other — a terminal signal, unlike a Triangle or Flag) — volatility/breakout patterns — **Volatility Squeeze** (Bollinger Bands contracting fully inside the Keltner Channel for a minimum number of consecutive days) and **NR4 / NR7 Narrow Range** (a bar whose True Range is the smallest of the trailing 4 or 7 bars and which is also a strict inside bar) — mean-reversion/exhaustion patterns — **Parabolic Stretch** (price stretched more than a configurable Z-score threshold away from its own 200-day SMA, measured against the distance series' own trailing mean/std) and **Bullish / Bearish Divergence** (price makes a new high/low while RSI fails to confirm it with a new high/low of its own) — and **Micro-Structure Candlestick Triggers**, 1-2 bar execution signals used to confirm a larger setup rather than as structural analysis in their own right: **Bullish Engulfing** / **Bearish Engulfing** (the current candle's body fully contains the opposite-colored prior candle's body) and **Hammer** / **Shooting Star** (a rejection wick at least 2x the real body with a negligible opposite wick) — each only counted when price is already near an extreme (below the lower Bollinger Band or RSI oversold for a bullish trigger, mirrored for bearish). Every candidate is checked against supporting signals — volume and RSI behaviour consistent with the setup, plus a pattern fit-quality score (R²) where a regression or piecewise shape applies. A pattern shows as **Forming** while its outcome is still undecided and **Confirmed** once price breaks through its key level, at which point a measured-move price target is calculated; Volatility Squeeze, NR4/NR7, and Symmetrical Triangle are genuinely non-directional while Forming — direction only resolves once a breakout actually occurs; Micro-Structure Candlestick Triggers are the one family with no Forming phase at all, since a 1-2 bar trigger either fired as of the latest bar or it didn't — it self-expires the day after, rather than lingering as a stale "active" pattern. A one-time "Backfill Historical Data" action walks two years of existing price history to populate a prediction-accuracy panel before relying on live alerts. All detected patterns, across every family, appear together in one ticker-grouped results table, filterable by Scope (Portfolio/Watchlist/All), Direction (Bullish/Bearish), and Family; clicking a ticker opens a dedicated chart page overlaying every one of its active patterns at once, with checkboxes to show/hide individual patterns or an entire Bullish/Bearish/Forming group, plus a plain-language explanation of each pattern shown. Every active pattern also appears as a color-coded tag (orange while Forming, red/green once Confirmed for bearish/bullish) on the Portfolio, Watchlist, and Stock Detail pages, alongside every other "Setups & Tags" badge. Scope (Portfolio default-on, Watchlist optional), per-family detector toggles, and alert thresholds are configurable in Settings → Tools → Pattern Detection.
* **Signal Pillar Confluence:** A cross-engine corroboration check, not a standalone page — it adds no new signal of its own, only flags when independent existing ones already agree. A ticker is flagged when at least two of three independent signal "pillars" (**Technical** — confirmed Pattern Detection results across every registered pattern family, plus Trap Monitor phase; **Statistical** — Earnings Volatility's historical post-earnings drift, only counted when there's a real measured mispricing edge; **ML** — the daily ML confidence score above/below 50) agree on direction within a 5-trading-day rolling window, with none of the three disagreeing. New pattern families are picked up automatically with no code changes, since the Technical pillar reads the same detector registry Pattern Detection itself is built on. Shows as a **📈/📉 Pillar Confluence** badge on Portfolio and Watchlist, a **Pillar Confluence** row on Stock Detail, and an optional "Pillar Confluence" column via the Columns picker.

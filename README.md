...
* **Macro Regime & Yield Curve Allocator:** A standalone tool (`/macro-allocator`) that synthesises live macro signals — 10y–2y yield curve spread, US CPI, high-yield credit spreads, the 10-year TIPS real yield, and the HMM hidden state — into one of five named economic regimes (Risk-On, Late Cycle, Stagflation, Contraction, Recovery). For each regime the tool shows the historically optimal asset class allocation (equities, bonds, commodities, cash) and, if Ghostfolio is connected, scores how closely your current portfolio matches it (0–100 alignment score) with a rebalancing delta table. The 90-day regime history is displayed as a timeline chart. Regime targets are configurable via the `REGIME_TARGETS` key in `config.json`. Requires the Macro Data Engine to have run at least once.

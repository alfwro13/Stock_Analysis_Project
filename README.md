...
* **FX Drag Analyzer:** Decomposes each US stock position's GBP return into two components: equity return in USD and FX effect from GBP/USD movement (`/fx-drag`). Shows which positions are genuinely outperforming and which are riding dollar strength. Covers YTD, 1-year, and 2-year periods using existing 2-year Parquet data — no additional data source required. A compact three-number FX breakdown (Equity USD / FX Effect / Total GBP YTD) also appears inline on each USD stock's detail page inside the "Your Position" box.

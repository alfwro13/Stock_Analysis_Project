...
* **Pattern Detection:** Scans Portfolio and Watchlist tickers once daily (`/pattern-detection`) for chart patterns using rolling-window swing-point detection, dispatched through a per-pattern-family detector registry so new pattern types can be added without touching the database schema, scheduler job, or chart renderer. Covers reversal patterns — **Head & Shoulders** / **Inverse Head & Shoulders** (a candidate is only accepted if the head genuinely exceeds both shoulders, the shoulders are roughly balanced in height and timing, and a genuine prior trend preceded the left shoulder) and **Double Top** / **Double Bottom** (two peaks or troughs of near-equal extreme separated by one qualifying opposite pivot) — and continuation patterns — **Bull Flag** / **Bear Flag** (a sharp flagpole move exceeding a volatility-scaled threshold, followed by a brief consolidation whose swing highs and lows form two roughly parallel, mildly counter-sloped channel lines) and **Ascending Triangle** / **Descending Triangle** (a flat resistance or support line against a rising or falling opposite side) — with more pattern types planned. Every candidate is checked against three supporting signals: volume should decline across the pattern's structural points then spike on the breakout, RSI divergence confirms weakening momentum, and a pattern fit-quality score (R²) ranks how cleanly price tracked the idealised shape. A pattern shows as **Forming** while price sits on the safe side of its key level and **Confirmed** once price breaks through it, at which point a measured-move price target is calculated. A one-time "Backfill Historical Data" action walks two years of existing price history to populate a prediction-accuracy panel before relying on live alerts. All detected patterns, across every family, appear together in one ticker-grouped results table, filterable by Scope (Portfolio/Watchlist/All), Direction (Bullish/Bearish), and Family; clicking a ticker opens a dedicated chart page overlaying every one of its active patterns at once, with checkboxes to show/hide individual patterns or an entire Bullish/Bearish group, plus a plain-language explanation of each pattern shown. Every active pattern also appears as a color-coded tag (orange while Forming, red/green once Confirmed for bearish/bullish) on the Portfolio, Watchlist, and Stock Detail pages, alongside every other "Setups & Tags" badge. Scope (Portfolio default-on, Watchlist optional), per-family detector toggles, and alert thresholds are configurable in Settings → Tools → Pattern Detection.

...
* **SMGB.L Morning Price Predictor:** A fixed-ticker predictor for the SMGB.L ETF at `/uk-etf-forecast`. Estimates the next-open price using the ETF's top-10 US holdings' post-close returns plus GBPUSD FX, with an OLS regression fallback and 60-day confidence interval. Run schedule configurable via `SCHEDULES.SMGB_PREDICTOR` in `config.json`.

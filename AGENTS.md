...
19. **Alert dedup gates must never auto-fire solely because a calendar day rolled over.** `IntradayOrchestrator._evaluate_alert_gate()` (`intraday_orchestrator.py`) is the single shared dedup/cooldown ledger (`alert_state` table) behind every alert engine — Crash, Moonshot, AI Contagion, Trap Monitor, HMM Regime, Market Stress, Macro, and HoldingLimit. A repeat alert for the same ticker/engine/condition must require either (a) a genuinely new condition class (the reason's fingerprint changed), or (b) cooldown elapsed **and** measurable worsening since the last fire. A UTC-date check alone is not a valid firing condition, even as a "first scan of the day" special case.

    **Composite key for per-account (not just per-ticker) conditions:** `alert_state`'s primary key is `(engine, ticker)` — one row per real ticker per engine. An engine whose condition is scoped *below* ticker level (e.g. a price target that's set independently per account, or any future per-account/per-holding condition) must not reuse the bare ticker symbol as the key, since that would collapse multiple independent conditions on the same ticker into one shared dedup state. Instead, encode the extra scope into the `ticker` column as a composite string (e.g. `f"{account_id}:{ticker}:{direction}"`) and pass that composite value everywhere `_evaluate_alert_gate`/`record_alert_fired` expects a ticker — no schema change needed, since the column is plain `TEXT` with no FK. The **HoldingLimit** engine (`intraday_orchestrator.py`, backing the Stock Detail page's "Set Targets" feature) is the reference implementation: it keys on `{account_id}:{ticker}:low` / `{account_id}:{ticker}:high` so a low-target breach and a high-target breach on the same ticker — and the same ticker held in two different accounts — each get independent cooldown/rearm state. When adding a direction- or scope-dependent worsened/recovered branch to `_evaluate_alert_gate` for such an engine, derive the direction from the composite key's suffix (see the `HoldingLimit` branch) rather than adding a second gate function.

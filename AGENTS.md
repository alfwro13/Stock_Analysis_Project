...
19. **Alert dedup gates must never auto-fire solely because a calendar day rolled over.** `IntradayOrchestrator._evaluate_alert_gate()` (`intraday_orchestrator.py`) is the single shared dedup/cooldown ledger (`alert_state` table) behind every alert engine — Crash, Moonshot, AI Contagion, Trap Monitor, HMM Regime, Market Stress, and Macro. A repeat alert for the same ticker/engine/condition must require either (a) a genuinely new condition class (the reason's fingerprint changed), or (b) cooldown elapsed **and** measurable worsening since the last fire. A UTC-date check alone is not a valid firing condition, even as a "first scan of the day" special case.

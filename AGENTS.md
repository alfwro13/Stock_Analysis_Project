...
- **Tooltips:** Use `<abbr title="Explanation text.">Label</abbr>` — wrap the label itself, no custom JS, no icon, no `style` attribute on the `<abbr>`. The global CSS handles `abbr` styling already. This is the established convention across all templates (e.g. Support 1, RSI, ATR). Keep tooltip text to 1–2 sentences matching that style.

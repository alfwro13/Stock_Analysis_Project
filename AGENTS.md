...
**Market-window checks:** Use `time_engine.is_market_open(exchange)` or `time_engine.market_window_utc(exchange)`. Never compare `datetime.now(timezone.utc).time()` against a hardcoded `"HH:MM"` string — that pattern is always a bug because the string has no timezone context. **Caveat:** `time_engine.is_market_open()`/`is_trading_session()` have no market-holiday calendar — they only know weekday + hours, so they report "open" on an exchange holiday that falls on a weekday (e.g. NYSE observing July 4th on the preceding Friday). For NYSE, LSE, XETRA, TSE, HKEX, SSE, ASX, or Euronext specifically, prefer `market_pulse.is_exchange_open(exchange)` instead — it reads the live Yahoo `marketState` cached from that exchange's proxy index ticker (`market_pulse._MARKET_STATUS_PROXY`: `^GSPC`/`^FTSE`/`^GDAXI`/`^N225`/`^HSI`/`000001.SS`/`^AXJO`/`^FCHI`, refreshed by `market_pulse.fetch_and_save_pulse()` into `market_pulse_cache.market_state`) and is therefore holiday-aware, falling back to the `time_engine` heuristic only when no live state has been cached yet. `GET /api/system/market-status`, `GET /api/system/market-status/all`, and `accounts_engine.tickers_needing_refresh()` already use it; any exchange outside that proxy list, and any caller needing premarket awareness (`include_premarket=True`), have no live proxy and must still use `time_engine` directly.

...
| `holding_price_limits` | Optional low/high price target per (account, ticker) — "Set Targets" on the Stock Detail page and Home Assistant's Low/High Limit entities read/write the same rows; built-in accounts only |
...
- **Set Targets — Low/High Position Targets** (`holding_price_limits` table via `db_accounts.py`, wired into `intraday_orchestrator.py`): user-defined low (buy) / high (sell) price targets per built-in account holding, added July 2026, surfaced as a "Set Targets" link on the Stock Detail page's Your Position box. The panel shows the existing ML Quantile Regression model's Q10/Q90 band (`ai_prediction_engine.py` — see the ML Quantile Price Bands glossary entry) as a starting suggestion; the user's own value is what's actually saved. A "Set for all accounts" checkbox applies the same pair to every built-in account holding that ticker via repeated calls to the existing `POST /api/accounts/holding-price-limit` (no new endpoint — this is the same endpoint Home Assistant's Low Limit/High Limit number entities already read/write, so a target set on either surface is reflected on the other with no extra sync code). Leaving a field blank or entering `0` clears that target (`db_accounts.upsert_holding_price_limit()` writes `NULL`). Every 5-minute intraday scan compares each held ticker's live price against any set target and fires a `holding_limit_alert` notification (source label "Position Target Reached") the first time it's crossed, via `IntradayOrchestrator._dispatch_holding_limit_alerts()` and the shared `alert_state` dedup gate — see rule 19's composite-key sub-rule for how a per-account, per-direction condition shares that ticker-keyed table without a schema change. Scoped to built-in accounts only; a Ghostfolio-only holding has no `account_id` to attach a target to.

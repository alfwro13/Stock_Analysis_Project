...
**Market-window checks:** Use `time_engine.is_market_open(exchange)` or `time_engine.market_window_utc(exchange)`. Never compare `datetime.now(timezone.utc).time()` against a hardcoded `"HH:MM"` string — that pattern is always a bug because the string has no timezone context. **Holiday calendar:** `time_engine.is_market_open()`/`is_trading_session()` are holiday-aware for every exchange in `exchange_hours.json` via `time_engine.is_exchange_holiday(exchange)`, backed by the `exchange_calendars` library (added 2026-07-13) — it vetoes a weekday+hours "open" result on a real scheduled holiday (e.g. NYSE observing July 4th on the preceding Friday). 26 of 29 exchanges have a dedicated calendar; NSE/SZSE/Euronext deliberately share a sibling exchange's calendar (`time_engine._EXCHANGE_CALENDAR_CODES`) since each genuinely follows the same trading-holiday calendar as its sibling. `market_pulse.is_exchange_open(exchange)` additionally layers live Yahoo `marketState` intraday session granularity (PRE/POST/REGULAR/CLOSED) on top for 8 exchanges (`market_pulse._MARKET_STATUS_PROXY`: NYSE/LSE/XETRA/TSE/HKEX/SSE/ASX/Euronext, refreshed by `market_pulse.fetch_and_save_pulse()` into `market_pulse_cache.market_state`) — prefer it over `time_engine` directly when you need that granularity or premarket awareness (`include_premarket=True`). `GET /api/system/market-status`, `GET /api/system/market-status/all`, and `accounts_engine.tickers_needing_refresh()` already use it.

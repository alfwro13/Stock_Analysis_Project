...
JOB_GRAPH: dict[str, dict] = {
    ...
    "overnight_quant_scan_job":       {"label": "Daily Quant Screener (Portfolio & Watchlist)",   "category": "quant",       "engine": "quant_engine.py",               "produces": ["quant_signals", "tail_risk", "earnings_drift_predictions"],  "consumes": ["historical_parquet"],                                               "settings_anchor": "automation-schedulers-card"},
    ...
    "weekend_earnings_vol_scan_job":  {"label": "Earnings Volatility Engine",                     "category": "quant",       "engine": "earnings_vol_engine.py",        "produces": ["earnings_volatility", "earnings_volatility_history"],        "consumes": ["historical_parquet", "yahoo_price_data"],                           "settings_anchor": "automation-schedulers-card"},
    "earnings_vol_retry_job":         {"label": "Earnings Volatility Retry",                      "category": "quant",       "engine": "earnings_vol_engine.py",        "produces": ["earnings_volatility", "earnings_volatility_history"],        "consumes": ["historical_parquet", "yahoo_price_data"],                           "settings_anchor": "automation-schedulers-card"},
    ...
    "ml_inference_job":               {"label": "Daily ML Inference",                             "category": "ml",          "engine": "ai_prediction_engine.py",       "produces": ["ml_predictions", "predicted_movers_history"],                "consumes": ["quant_signals", "ml_model"],                                        "settings_anchor": "ml-ai-card"},
    ...
    "risk_orchestrator_job":          {"label": "Risk Orchestrator Scan",                         "category": "risk",        "engine": "risk_orchestrator_engine.py",   "produces": ["portfolio_heat_index", "ticker_risk_contribution"],          "consumes": ["xray_caches", "quant_signals", "stock_signals", "portfolio"],       "settings_anchor": "risk-orchestrator-card"},
    "risk_orchestrator_digest_job":   {"label": "Risk Orchestrator Daily Rollup",                 "category": "risk",        "engine": "risk_orchestrator_engine.py",   "produces": [],                                                             "consumes": ["portfolio_heat_index", "atr_stop_history"],                        "settings_anchor": "risk-orchestrator-card"},
    ...
    "pattern_detection_job":           {"label": "Pattern Detection",                              "category": "quant",       "engine": "pattern_detection_engine.py",   "produces": ["pattern_detection_results", "pattern_detection_history"],     "consumes": ["historical_parquet", "portfolio"],                                  "settings_anchor": "tools-card"},
    "pattern_detection_accuracy_fill_job": {"label": "Pattern Detection (accuracy fill)",          "category": "quant",       "engine": "pattern_detection_engine.py",   "produces": ["pattern_detection_history"],                                  "consumes": ["pattern_detection_history", "historical_parquet"],                  "settings_anchor": "tools-card"},
    ...
    # Predicted Movers — not a scheduled job; the leaderboard is a live SELECT over
    # already-computed quant_signals quantile bands, and the accuracy page reads the
    ...
    # Signal Pillar Confluence — not a scheduled job; a read-only per-ticker check over
    # already-persisted Pattern Detection/Trap Monitor/Earnings Volatility/ML rows, rendered
...
CONFIG_KEY_TO_JOB: dict[str, str] = {
    ...
    "RISK_ORCHESTRATOR":  "risk_orchestrator_job",
    "RISK_ORCHESTRATOR_DIGEST": "risk_orchestrator_digest_job",
